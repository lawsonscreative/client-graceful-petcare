        ...
        {services.map((service, index) => {
          ...
          return (
            <section
              key={service.id}
              id={service.id}
              className={`py-20 lg:py-24 ${bgColor} scroll-mt-24`}
            >
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
                  {/* Image - appears first on mobile, alternates on desktop */}
                  <div className={`relative h-[400px] lg:h-[500px] rounded-2xl overflow-hidden shadow-xl ${!isEven ? 'lg:order-2' : ''}`}>
                    <Image
                      ...
                      <ul className="space-y-2">
                        {service.features.map((feature, idx) => (
                          <li key={idx} className="flex items-start">
                            <Check className="w-5 h-5 text-primary mr-3 mt-0.5 flex-shrink-0" aria-hidden="true" />
